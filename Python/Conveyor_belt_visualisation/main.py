...
while running:
    ...
    # Make velocity alternate between 0 and 0.6m/s
    if direction:
        con_velocity += 0.03
        if con_velocity > max_velocity:
            direction = False
    else:
        con_velocity -= 0.03
        if con_velocity < 0:
    ...
    # Draw text
    draw_text((screenX * (1/4), screenY*(1/2)), str(round((fps * con_velocity) / con_length, 1)) + " m/s")
    draw_text((screenX * (1/2), screenY*(1/3)), str(round(brightness)) + "%")
    draw_text((screenX * (6/8), screenY * (1/5)), str(round(((fps * con_velocity) / con_length)*(10/2), 1)) + " kHz")
